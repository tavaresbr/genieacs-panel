...
class SgpLink {
  ...
  static async upsert(link) {
    const now = new Date();
    // The conflict target has to name both columns now that the unique is
    // `(tenant_id, device_id)`: with only `device_id` Postgres has no matching
    // index and raises, and MySQL would happily merge onto another provider's
    // row for the same GenieACS device id.
