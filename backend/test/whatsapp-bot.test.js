...
before(async () => {
  ...
  // The panel's public address, and therefore the portal link the bot sends.
  await WhatsAppConfigService.saveConfig({ webhookBaseUrl: `${PORTAL_BASE}/api/whatsapp-webhook` });
...
describe('the line the bot must not cross', () => {
  ...
  it('hands off instead of sending a broken link when no public URL is configured', async () => {
    await WhatsAppConfigService.saveConfig({ webhookBaseUrl: '' });
    try {
